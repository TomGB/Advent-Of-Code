...
const run = (fileName, markerLength) => {
	const { grid: initialGrid } = loadData(fileName);

	const possibleStartingLocations = initialGrid
		.map(row => row.filter(({ heightLetter }) => heightLetter === "a"))
		.flat();

	const grid = JSON.parse(JSON.stringify(initialGrid));

	let currentSteps = possibleStartingLocations;
	...
	do {
		...
		numSteps++;
	} while (currentSteps != "finished" && numSteps < 340);
